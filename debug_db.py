...
for s in Student.objects.all():
    print(f"Name: {s.name}, Email: {s.email}, Dept: {s.department}, Programme: {s.programme}, Year: {s.passed_out_year}")

print("\n--- Jobs (Active) ---")
for j in Job.objects.filter(is_active=True):
    print(f"Title: {j.title}, Company: {j.company.name}, Depts: {j.departments}, Progs: {j.programmes}, Years: {j.graduation_years}")
