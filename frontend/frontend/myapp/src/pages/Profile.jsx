...
const Profile = () => {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      profileData.profile_image = normalizeMediaUrl(profileData.profile_image);
      profileData.resume = normalizeMediaUrl(profileData.resume);

      profileData.certificates = profileData.certificates.map(cert => ({
        ...cert,
        certificate_file: normalizeMediaUrl(cert.certificate_file)
      }));
      ...
      try {
        ...
        console.warn("Could not load social links:", socialErr);
      }
  ...
  const submitModal = async () => {
    ...
    switch (modalType) {
      ...
      case "addCert":
      case "editCert":
        if (!modalData.title || (modalType === "addCert" && !modalData.file)) {
          alert("Title and file are required for new certificate");
          setLoading(false);
          return;
        }
    ...
    <>
      ...
      <div className="profile-container">
        ...
        <div className="profile-header-card">
          <div className="profile-header">
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", width: "100%", maxWidth: "180px" }}>
                <button
                  className="action-btn action-btn-primary"
                  onClick={() => document.getElementById('student-photo-input').click()}
                >
            ...
            <div className="profile-info">
              ...
              <div style={{ marginTop: "1.5rem" }}>
                <div className="info-row">
                  <div className="label-wrapper"><i className="bi bi-envelope-fill"></i><strong>Email:</strong></div>
                  <span className="info-value">{student.email || "—"}</span>
                </div>
                <div className="info-row">
                  <div className="label-wrapper"><i className="bi bi-person-badge-fill"></i><strong>Reg No:</strong></div>
                  <span className="info-value">{student.university_reg_no || "—"}</span>
                </div>
                <div className="info-row">
                  <div className="label-wrapper"><i className="bi bi-telephone-fill"></i><strong>Phone:</strong></div>
                  <span className="info-value">{student.phone || "Not provided"}</span>
                </div>
                <div className="info-row">
                  <div className="label-wrapper"><i className="bi bi-building-fill"></i><strong>Department:</strong></div>
                  <span className="info-value">{student.department || "—"}</span>
                </div>
                <div className="info-row">
                  <div className="label-wrapper"><i className="bi bi-calendar-event-fill"></i><strong>Passed Out:</strong></div>
                  <span className="info-value">{student.passed_out_year || "—"}</span>
                </div>
              </div>
            </div>
          ...
          {profile.education?.length > 0 ? (
            profile.education.map((edu) => (
              <div key={edu.id} className="item-row">
                ...
                <div className="item-actions">
                  <button className="action-btn action-btn-outline" onClick={() => openModal("editEdu", edu)}>
                    <i className="bi bi-pencil-square me-1"></i> Edit
                  </button>
                  <button className="action-btn action-btn-danger" onClick={() => deleteEducation(edu.id)}>
                    <i className="bi bi-trash me-1"></i> Delete
                  </button>
                </div>
              </div>
            ...
            <p style={{ color: "#64748b", fontStyle: "italic" }}>No education entries yet.</p>
          )}
          <button className="action-btn action-btn-primary" onClick={() => openModal("addEdu")}>
            <i className="bi bi-plus-circle-fill me-2"></i> Add Education
          </button>
        </div>
        ...
        <div className="profile-card">
          ...
          {profile.certificates?.length > 0 ? (
            profile.certificates.map((cert) => (
              <div key={cert.id} className="item-row">
                <div>
                  ...
                  <div className="certificate-meta">
                    {cert.issued_by && (
                      <span style={{ background: "#f3e8ff", color: "#6A0DAD", padding: "0.35rem 0.9rem", borderRadius: "8px", fontWeight: 600 }}>
                        <i className="bi bi-award me-2"></i>{cert.issued_by}
                      </span>
                    )}
                    {cert.year_obtained && (
                      <span style={{ background: "#f3e8ff", color: "#6A0DAD", padding: "0.35rem 0.9rem", borderRadius: "8px", fontWeight: 600 }}>
                        <i className="bi bi-calendar-event me-2"></i>{cert.year_obtained}
                      </span>
                    )}
                  </div>
                  ...
                  {cert.certificate_file && (
                    <div style={{ marginTop: "1.2rem" }}>
                      <a href={cert.certificate_file} target="_blank" rel="noreferrer">
                        <div className="preview-container">
                          {cert.certificate_file.toLowerCase().endsWith('.pdf') ? (
                            <div className="preview-pdf">
                              <i className="bi bi-file-earmark-pdf-fill" style={{ fontSize: "3rem" }}></i>
                            ...
                            <img
                              ...
                              onError={(e) => {
                                ...
                                e.target.parentElement.innerHTML = `
                                  <div class="preview-pdf" style="color:#64748b;">
                ...
                <div className="item-actions">
                  <button className="action-btn action-btn-outline" onClick={() => openModal("editCert", cert)}>
                    <i className="bi bi-pencil-square me-1"></i> Edit
                  </button>
                  <button className="action-btn action-btn-danger" onClick={() => deleteCertificate(cert.id)}>
                    <i className="bi bi-trash me-1"></i> Delete
                  </button>
                </div>
              </div>
            ...
            <p style={{ color: "#64748b", fontStyle: "italic" }}>No certificates added yet.</p>
          )}

          <button className="action-btn action-btn-primary" onClick={() => openModal("addCert")}>
            <i className="bi bi-plus-circle-fill me-2"></i> Add Certificate
          </button>
        </div>
        ...
        <div className="profile-card">
          ...
          {profile.resume ? (
            <>
              <a href={profile.resume} target="_blank" rel="noreferrer" style={{ textDecoration: "none" }}>
                <div className="preview-container">
                  <div className="preview-pdf">
                    <i className="bi bi-file-earmark-pdf-fill" style={{ fontSize: "3.5rem" }}></i>
                    <div style={{ fontWeight: 600 }}>Click to View Resume</div>
                    <small>(PDF document)</small>
                  </div>
                </div>
              </a>

              <div className="resume-actions" style={{ marginTop: "1.5rem", display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                <button className="action-btn action-btn-primary" onClick={() => openModal("replaceResume")}>
                  <i className="bi bi-arrow-repeat me-2"></i> Replace
                </button>
              </div>
            </>
            ...
            <>
              <p style={{ color: "#64748b", fontStyle: "italic" }}>No resume uploaded yet.</p>
              <button
                className="action-btn action-btn-primary"
        ...
        <div className="profile-card">
          ...
          Object.values(profile.social_links).some(link => link?.trim()) ? (
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {profile.social_links.github && (
                <a href={profile.social_links.github} target="_blank" rel="noopener noreferrer" className="social-link">
                  <i className="bi bi-github social-icon"></i> GitHub
                </a>
              )}
              {profile.social_links.linkedin && (
                <a href={profile.social_links.linkedin} target="_blank" rel="noopener noreferrer" className="social-link">
                  <i className="bi bi-linkedin social-icon"></i> LinkedIn
                </a>
              )}
              {profile.social_links.portfolio && (
                <a href={profile.social_links.portfolio} target="_blank" rel="noopener noreferrer" className="social-link">
                  <i className="bi bi-globe social-icon"></i> Portfolio
                </a>
              )}
              {profile.social_links.twitter && (
                <a href={profile.social_links.twitter} target="_blank" rel="noopener noreferrer" className="social-link">
                  <i className="bi bi-twitter-x social-icon"></i> Twitter / X
                </a>
              )}
            </div>
          ) : (
            <p style={{ color: "#64748b", fontStyle: "italic" }}>No social links added yet.</p>
          )}
      ...
      {showRemovePhotoModal && (
        <div style={{
          ...
          <div style={{
            background: 'white', borderRadius: '16px', width: '90%', maxWidth: '420px',
            boxShadow: '0 20px 60px rgba(0,0,0,0.3)', overflow: 'hidden'
          }}>
            <div style={{
              background: 'linear-gradient(135deg, #4B0082, #6A0DAD)',
              color: 'white', padding: '1.4rem 1.8rem', display: 'flex',
              justifyContent: 'space-between', alignItems: 'center'
            }}>
              <h5 style={{ margin: 0 }}><i className="bi bi-trash me-2"></i>Remove Profile Photo</h5>
              <button onClick={() => setShowRemovePhotoModal(false)} style={{ background: 'none', border: 'none', color: 'white', fontSize: '1.8rem' }}>×</button>
            </div>

            <div style={{ padding: '2rem 1.8rem', textAlign: 'center' }}>
              <i className="bi bi-exclamation-circle-fill" style={{ color: '#e53e3e', fontSize: '2.8rem', marginBottom: '1rem' }}></i>
              <p style={{ fontWeight: 600, fontSize: '1.15rem', marginBottom: '0.5rem' }}>
                Are you sure you want to remove your profile photo?
              </p>
            ...
            <div style={{
              padding: '1.2rem 1.8rem', background: '#fafbfc', borderTop: '1px solid #e2e8f0',
              display: 'flex', gap: '1rem', justifyContent: 'flex-end'
            }}>
              <button onClick={() => setShowRemovePhotoModal(false)} style={{ padding: '0.6rem 1.4rem', borderRadius: '8px', border: '1px solid #ccc', background: 'white' }}>
                Cancel
              </button>
              <button
                onClick={confirmRemoveProfilePhoto}
                disabled={removingPhoto}
                style={{
                  padding: '0.6rem 1.4rem', borderRadius: '8px', background: '#dc3545', color: 'white',
      ...
      {modalType && (
        <div className="modal fade show" style={{ display: "block", backgroundColor: "rgba(0,0,0,0.55)" }} tabIndex="-1">
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {modalType.includes("add") ? "Add" : "Edit"} {modalType.includes("Edu") ? "Education" : modalType.includes("Cert") ? "Certificate" : modalType.includes("About") ? "About" : modalType.includes("Social") ? "Social Links" : "Resume"}
                </h5>
                <button type="button" className="btn-close" onClick={closeModal}></button>
              </div>
              ...
              <div className="modal-body">
                {["addEdu", "editEdu"].includes(modalType) && (
                  <>
                    <div className="mb-3">
                      <label className="modal-form-label">Degree *</label>
                      <input type="text" className="form-control" name="degree" value={modalData.degree || ""} onChange={handleModalChange} />
                    </div>
                    <div className="mb-3">
                      <label className="modal-form-label">Institution *</label>
                      <input type="text" className="form-control" name="institution" value={modalData.institution || ""} onChange={handleModalChange} />
                    </div>
                    <div className="row g-3">
                      <div className="col-md-6">
                        <label className="modal-form-label">Year of Passing</label>
                        <input type="text" className="form-control" name="year_of_passing" value={modalData.year_of_passing || ""} onChange={handleModalChange} />
                      </div>
                      <div className="col-md-6">
                        <label className="modal-form-label">CGPA</label>
                        <input type="text" className="form-control" name="cgpa" value={modalData.cgpa || ""} onChange={handleModalChange} />
                      </div>
                    </div>
                  </>
                )}
                ...
                {["addCert", "editCert"].includes(modalType) && (
                  <>
                    <div className="mb-3">
                      <label className="modal-form-label">Certificate Title *</label>
                      <input type="text" className="form-control" name="title" value={modalData.title || ""} onChange={handleModalChange} />
                    </div>
                    <div className="mb-3">
                      <label className="modal-form-label">Issued By</label>
                      <input type="text" className="form-control" name="issued_by" value={modalData.issued_by || ""} onChange={handleModalChange} />
                    </div>
                    <div className="mb-3">
                      <label className="modal-form-label">Year Obtained</label>
                      <input type="text" className="form-control" name="year_obtained" value={modalData.year_obtained || ""} onChange={handleModalChange} />
                    </div>
                    <div className="mb-3">
                      <label className="modal-form-label">
                        {modalType === "addCert" ? "Upload Certificate *" : "Replace Certificate (optional)"}
                ...
                {modalType === "editAbout" && (
                  <div className="mb-3">
                    <label className="modal-form-label">About Yourself</label>
                    <textarea className="form-control" rows="5" name="about" value={modalData.about || ""} onChange={handleModalChange} />
                  </div>
                )}
                ...
                {modalType === "editSocial" && (
                  <>
                    <div className="mb-3"><label className="modal-form-label">GitHub URL</label><input type="url" className="form-control" name="github" value={modalData.github || ""} onChange={handleModalChange} /></div>
                    <div className="mb-3"><label className="modal-form-label">LinkedIn URL</label><input type="url" className="form-control" name="linkedin" value={modalData.linkedin || ""} onChange={handleModalChange} /></div>
                    <div className="mb-3"><label className="modal-form-label">Portfolio URL</label><input type="url" className="form-control" name="portfolio" value={modalData.portfolio || ""} onChange={handleModalChange} /></div>
                    <div className="mb-3"><label className="modal-form-label">Twitter / X URL</label><input type="url" className="form-control" name="twitter" value={modalData.twitter || ""} onChange={handleModalChange} /></div>
                  </>
                )}
              </div>
              ...
              <div className="modal-footer">
                ...
                <button type="button" className="btn btn-primary" onClick={submitModal} disabled={loading}>
