  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:8000/companies/register/",
        form
      );

      // Updated success message + redirect to LOGIN page
      setMessage(res.data.message || "Registration successful! Please log in to continue.");

      // Redirect to login page (not pending)
      setTimeout(() => {
        navigate("/company-login", { state: { email: form.email } });
      }, 1800);
    } catch (err) {
    ...
    <>
      ...
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .register-title {
          font-size:2rem; font-weight:700; color:#1e293b; margin-bottom:0.3rem;
        }

        .register-subtitle { color:#475569; font-size:1rem; }
        ...
        .form-control {
          ...
          font-size:0.98rem; background:#f8fafc;
        ...
        .alert-custom {
          ...
          border-radius:1rem; padding:1.1rem 1.3rem; margin-bottom:1.5rem;
          display:flex; align-items:center; gap:0.8rem;
          font-size:0.96rem; animation:shake 0.45s ease-in-out;
        }
        ...
        .register-text {
          ...
          font-size:0.97rem; color:#475569;
        }

        .register-link {
          color:var(--primary-start); font-weight:600; cursor:pointer;
        }

        .register-link:hover { text-decoration:underline; color:#4f46e5; }

        @media (max-width:576px) {
          .register-wrapper { padding:1.6rem 1.4rem; border-radius:1.3rem; max-width:90%; }
          .register-title { font-size:1.7rem; }
          .register-subtitle { font-size:0.92rem; }
          .form-label { font-size:0.9rem; }
          .form-control { height:48px; font-size:0.94rem; padding:0 2.8rem 0 2.9rem; }
          .input-icon { font-size:1.2rem; }
          .btn-register { font-size:1rem; padding:0.8rem; }

          .star { border-left:20px solid transparent !important; border-right:20px solid transparent !important; border-bottom:40px solid rgba(255,255,255,0.12) !important; }
          .star::before, .star::after { border-left:20px solid transparent !important; border-right:20px solid transparent !important; border-bottom:40px solid rgba(255,255,255,0.12) !important; }
          .star.small { border-left:14px solid transparent !important; border-right:14px solid transparent !important; border-bottom:28px solid rgba(255,255,255,0.10) !important; }
          .star.small::before, .star.small::after { border-left:14px solid transparent !important; border-right:14px solid transparent !important; border-bottom:28px solid rgba(255,255,255,0.10) !important; }
        }

        @media (max-width:360px) {
          .register-wrapper { padding:1.4rem 1.2rem; }
      ...
      <div className="page">
        ...
        <div className="register-wrapper">
          <div className="logo-section">
            ...
            <h1 className="register-title">Company Registration</h1>
