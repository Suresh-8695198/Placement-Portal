...
const CompanyVerificationRequest = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.email || !formData.name || !formData.contact) {
    ...
    try {
      const res = await axios.post(
        'http://localhost:8000/companies/request-verification/',
        formData,
        {
      ...
      setMessage(res.data.message || 'Verification request submitted successfully!');

      // ────────────────────────────────────────────────
      // Redirect to COMPANY verification pending page
      // (not the student one)
      setTimeout(() => {
        navigate('/company-verification-pending', { state: { email: formData.email } });
      }, 2000);
      ...
      <style>{`
        ...
        * {
        ...
        @media (max-width: 360px) {
          .verification-wrapper { padding: 1.4rem 1.2rem; }
      ...
      <div className="page">
        ...
        <div className="verification-wrapper">
          <div className="logo-section">
            ...
            <h1 className="title">Request Company Verification</h1>
