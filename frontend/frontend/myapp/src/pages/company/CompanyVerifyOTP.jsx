  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:8000/companies/verify-otp/",
        { email, otp }
      );
      ...
      setTimeout(() => {
        navigate("/company-reset-password", { state: { email } });
      }, 1000);
    } catch (err) {
      ...
      <style>{`
        ...
        * {
        ...
        @media (max-width: 576px) {
          ...
          .btn-verify { font-size: 1rem; padding: 0.8rem; }
        ...
        @media (max-width: 360px) {
          .otp-wrapper { padding: 1.4rem 1.2rem; }
      ...
      <div className="page">
        ...
        <div className="otp-wrapper">
          ...
          <div className="back-link">
            Didn't receive OTP?{" "}
            <span onClick={() => navigate("/company-forgot-password")}>
              Resend
            </span>{" "}
            |{" "}
            <span onClick={() => navigate("/company-login")}>Back to Login</span>
          </div>
        </div>
      </div>
    </>
